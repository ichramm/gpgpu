                                 ...
                                 value_type *y)
{
    ...
    if (row < mat.rows) {
        ...
        for (auto j = mat.row_pointers[row]+lane,
                  end = mat.row_pointers[row+1]; j < end; j += 32) {
            acc += mat.values[j] * x[mat.col_indices[j]];
        }
...
static void initial_kindergarten_test()
{
    ...
    value_type h_vecY[rows] = {0, 0, 0, 0, 0, 0};
    ...
    print_vector("vectX", h_vecX, columns);

    ser_spmv_kernel_host(mat, h_vecX, h_vecY);
    print_vector("vectY", h_vecY, rows);
...
void ejercicio1() {
    ...
    lab_tests_controller([&](float p, uint32_t rows, uint32_t columns) {
        CSRMatrix<value_type> mat{rows, rows};
        mat.random_init(p);
        ...
        auto dMat = mat.to_device();
        auto d_vecX = dev_alloc_fill(columns, h_vecX.data());
        auto d_vecY = dev_alloc_zero<value_type>(rows);
        auto d_expectedResult = dev_alloc<value_type>(rows);
        ...
        CUDA_CHK(cudaMemcpy(d_vecY.get(), h_vecY.data(), 1, cudaMemcpyHostToDevice));
        ...
        run_kernel("par_spmv_kernel4", [&]() {
            dim3 dimGrid{rows / (BLOCK_SIZE/32)};
            dim3 dimBlock{BLOCK_SIZE};
